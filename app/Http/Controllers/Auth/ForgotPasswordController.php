    ...
    public function postrecoverpassword(Request $request)
    {
        ...
        if ($result) {
            ...
            Mail::send('user.user_layout.mail', $data, function ($message) use ($email) {
                $message->from('ngovangiahuy04@gmail.com', '12C4Movies@no-reply');
                $message->to($email, 'Visitor')->subject('Email khôi phục tài khoản 12C4Movies!');
            });
    ...
    public function postRecoverNewPassword(Request $request, $token)
    {
        ...
        if ($result) {
            if ($request->password == $request->repassword) {
                ...
                $user->save();
                return redirect()->route('user.index')->with(['thongbao_level' => 'success', 'thongbao' => "<b>Khôi phục mật khẩu thành công!</b><br>Hãy tận hưởng những phút giây thư giãn trên <b>12C4Movies</b>"]);
            } else
